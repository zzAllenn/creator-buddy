...
**RNote 路线**：结果是平台搜索样本，无红狐 1000+ 收录门槛、T+1 更新或固定 30 天库口径。默认一周内、最多点赞排序；仅支持 `一天内 / 一周内 / 半年内 / 不限`。返回的互动数保留原值，缺失值为 `null`，不能当 0；无三维评分，不估算粉丝数或发布时间。搜索卡片没有现成 URL 时，适配器使用原始 `note.id` 和 `note.xsec_token` 生成带 `xsec_source=pc_search` 的笔记链接；两者缺一才标注「链接缺失」。单次搜索不是全站统计，空结果不能推断赛道冷却。
...
- **链接口径**：接口有 `note_url` / `noteLink` 时必须原样输出。RNote 搜索卡片只有 `note.id + note.xsec_token` 时，统一适配器生成 `https://www.xiaohongshu.com/explore/{note_id}?xsec_token={URL编码后的原始token}&xsec_source=pc_search`。禁止丢掉或修改 token，也禁止只有 `note_id` 时生成链接。`note_id` 必须是完整的 24 位小写十六进制。
...
- 有完整 URL 或 RNote 已生成链接时，标题做成 Markdown 链接；缺少 ID/token 时显示纯标题并标注链接缺失。链接必须保留 `xsec_token`，作者有 ID 时链接 `https://www.xiaohongshu.com/user/profile/{authorId}`。
...
- **链接必须带有效 token**。已有完整 URL 不改写；RNote 仅在 ID 和 `xsec_token` 都存在时生成链接。丢掉 token 或只用 ID 生成的链接不可交付。
